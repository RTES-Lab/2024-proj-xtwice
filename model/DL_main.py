...
def main(yaml_config, target_config, save_model=False, save_log=False):
    ...
    accuracies, losses, all_y_true, all_y_pred, best_model, best_fold_history = trainer.get_best_model(model, dataloader)
    ...
    # Best model 저장
    torch.save(best_model.state_dict(), "./tmp/best_model.pth")


    best_model = model.get_model(n_classes=4)
    best_model.load_state_dict(torch.load("./tmp/best_model.pth", weights_only=True))
    ...
    try:
        ...
        optimized_scripted_module._save_for_lite_interpreter(model_path)
...
def load_csv_data2(csv_path, segment_length=2048):
    import pandas as pd
    # CSV 파일에서 데이터 로드
    data = pd.read_csv(csv_path).iloc[:, 0].values
...
def predict_with_ptl(model_path, csv_path, yaml_config):
    ...
    # 데이터 로드
    segments = load_csv_data(csv_path)
