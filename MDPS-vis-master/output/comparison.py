...
def get_data(root_dir, folder_list:list, csv_file):
    ...
    for subdir in selected_folders:
        ...
        # CSV 파일을 불러옵니다.
        df = pd.read_csv(file_path, usecols=[0,1], names=['A','B'], header=0)  # A열만 불러옵니다.
...
def compare_data(data, column : str, csv_file):
    ...
    plt.ylim(-0.1, 0.1)
    plt.legend()  # 범례 추가
    plt.savefig(os.path.join(root_dir, f'comparison_{column}_H_and_B_{csv_file[0].upper()}_axis.png'))  # 그래프 저장
...
root_dir = '/home/kseorang/Github/X-twice/MDPS-vis-master/output'
selected_folders = ['0530_30204_B_1200RPM_120fps_7','0530_30204_H_1200RPM_120fps_7']
