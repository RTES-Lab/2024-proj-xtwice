...
def get_displacment():
    ...
    video_path = f"../../../input/{filename}"  # 동영상 파일 경로 입력
...
def get_graph(csv_file : str, mm_per_pixel):
    ...
    df = pd.read_csv(file, usecols=[0, 1], names=['A', 'B'], header=0)  # 첫 4개 열만 불러옵니다.
...
def main():
    ...
    get_graph('z.csv', mm_per_pixel)

if __name__ == "__main__":

    real_length = 65

    filename = '0724_30204_OR_1200.mov'
    if filename[-1] == "7":
        real_length = 30
    elif filename[-1] == "1":
        real_length = 65
